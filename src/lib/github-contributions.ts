...
export async function getGithubContributions(
  ...
  try {
    ...
    if (!tbodyMatch) throw new Error("Contribution table body not found");

    const rowRegex = /<tr[^>]*>([\s\S]*?)<\/tr>/g;
    const cellRegex =
      /data-ix="(\d+)"[\s\S]*?data-date="([^"]+)"[\s\S]*?data-level="(\d+)"/g;

    const rows: { ix: number; date: string; level: number }[][] = [];
    for (const rowMatch of tbodyMatch[1].matchAll(rowRegex)) {
      const entries = Array.from(rowMatch[1].matchAll(cellRegex)).map(
        (cellMatch) => ({
      ...
      if (entries.length > 0) rows.push(entries);
    }
    ...
    const weeks: ContributionDay[][] = Array.from(
      { length: weekCount },
      () =>
